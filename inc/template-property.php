        ...
        if (!empty($features_terms)) {
            foreach ($features_terms as $features_term) {
                if (isset($_GET['feature-' . $features_term->slug]) && $_GET['feature-' . $features_term->slug] == 'on') {
                    echo '<p class="features-checkbox"><input checked="checked" class="checkbox" type="checkbox" id="feature-' . $features_term->slug . '" name="feature-' . $features_term->slug . '" /><label for="feature_' . $features_term->slug . '">' . $features_term->name . '</label><span>' . $features_term->count . '</span></p>';
                } else {
...
function property_loadmore_ajax_handler()
{
    ...
    $stemplate = 'template-parts/' . $property_load->post->post_type . '/item';

    if ($property_load->have_posts()) :
        while ($property_load->have_posts()) : $property_load->the_post();
            get_template_part($stemplate);
        endwhile;
    endif;
...
function property_filter_function()
{
    ...
    if ($property_filter->have_posts()) :
        ...
        while ($property_filter->have_posts()) : $property_filter->the_post();
    ...
    echo json_encode(array(
        'posts' => json_encode($property_filter->query_vars),
